...
MISSIONLIST =  [{'name':'산소', 'ans':['O.png', 'O.png']}, {'name':'질소', 'ans':['N.png', 'N.png']},
                {'name':'물', 'ans':['H.png', 'H.png', 'O.png']}, {'name':'이산화탄소', 'ans':['C.png', 'O.png', 'O.png']},
                {'name':'이산화황', 'ans':['S.png', 'O.png', 'O.png']}, {'name':'삼불화붕소', 'ans':['B.png', 'F.png', 'F.png', 'F.png']},
                {'name':'일산화탄소', 'ans':['C.png', 'O.png']}, {'name':'시레인', 'ans':['Si.png', 'H.png', 'H.png', 'H.png', 'H.png']},
...
pygame.display.set_caption('Elements Combination GAME')
...
Errorfont = pygame.font.Font('HoonWhitecatR.ttf', 45)
...
def main():
    ...
    Error1 = Errorfont.render('원소를 잘못먹었다..', True, LIGHTRED)
    Error1Rect = Error1.get_rect()
    Error1Rect.center = (WINDOWWIDTH / 2, WINDOWHEIGHT / 2)
    

    # 미션
    MissionInt = random.randint(0, len(MISSIONLIST)-1)
    Mission = Missionfont.render('MISSION: %s을(를) 만들어라!' %(MISSIONLIST[MissionInt]['name']), True, BLACK)
    MakeAnsList(MISSIONLIST[MissionInt]['ans'], MissionAns)
    ...
    while True:
        ...
        for e in ELEMENTS:
            if BeakerRect.colliderect(e['rect']):
                ELEMENTS.remove(e)
                if not e['ansName'] in MissionAns:
                    Score -= 1
                    GameWindow.blit(Error1, Error1Rect)
...
def StartScreen():
    ...
    GameNameRect.center = (int(WINDOWWIDTH / 2), int(WINDOWHEIGHT / 3) - 15)
    
    pressKey = Scorefont.render('Press a key to play', True, LIGHTRED)
    pressKeyRect = pressKey.get_rect()
    pressKeyRect.center = (int(WINDOWWIDTH / 2), int(WINDOWHEIGHT / 3) + 35)
...
def instruction():
    ...
    instruction1 = Scorefont.render('2. Press the Left(A) or Right(D) key', True, BLACK)
    instruction1_ = Scorefont.render('to move the beaker to the left or right.', True, BLACK)
    instruction2 = Scorefont.render('3. Combine the elements!', True, BLACK)
    instruction3 = fpsfont.render('Press the ESC to continue', True, LIGHTRED)

    GameWindow.blit(instructionBG, (0,0))
    GameWindow.blit(instructionTitle, (5, 70))
    GameWindow.blit(instruction0, (5,140))
    GameWindow.blit(instruction1, (5,190))
    GameWindow.blit(instruction1_, (50,240))
    GameWindow.blit(instruction2, (5,290))
    GameWindow.blit(instruction3, (700, 300))
    while checkForKeyPress() != K_ESCAPE:
        KeyEvent()
        pygame.display.flip()

def MakeAnsList(ans, List):
    for i in ans:
        List.append(i)
